    ...
    <div class="bg-white shadow-md rounded-lg p-4">
      ...
      <ul class="list-disc list-inside">
        ...
        <li class="mb-2 flex items-center">
          @if($isConfigured)
          <img src="{{ Vite::asset('resources/svg/' . $apiName . '-color.svg') }}" alt="{{ ucfirst($apiName) }} Logo" class="mr-2 h-6 w-6">
          <span>{{ ucfirst($apiName) }} API: <span class="font-medium text-green-600">Configured</span></span>
          @else
          <img src="{{ Vite::asset('resources/svg/' . $apiName . '-color.svg') }}" alt="{{ ucfirst($apiName) }} Logo" class="mr-2 h-6 w-6 opacity-50">
